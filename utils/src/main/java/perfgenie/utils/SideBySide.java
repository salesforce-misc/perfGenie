...
public class SideBySide {
    ...
    public static long maxTimeWindow = 12 * 60 * 60 * 1000; // 4 hours due to argus query limitations, need to switch to huron
    ...
    public static synchronized CanaryResponse processSideBySideCanaryTask(long timestampStart, long timestampEnd, String cell, int type, String basekpods, String canarykpods) {
        try{
            ...
            System.out.println(scope + ":" + instance + ":" + domain + ":basekpods=" + basekpods + ":canarykpods=" + canarykpods);
            ...
            CanaryDetails canary = processCanary(String.valueOf(timestampStart), String.valueOf(timestampEnd), instance, domain, cell, basekpodsValue, canarykpodsValue);
            if (canary != null && canary.pod1.size() == canary.pod2.size()) {
                ...
                futures2.add(pool.submitTask(() -> ArgusQueryT.getArgusMetric("reqCount", canary.finalStart, canary.finalEnd, instance, domain, cell, canary.pod1)));
                futures2.add(pool.submitTask(() -> ArgusQueryT.getArgusMetric("reqCount", canary.finalStart, canary.finalEnd, instance, domain, cell, canary.pod2)));
                ...
                futures2.add(pool.submitTask(() -> ArgusQueryT.getStatupAVG(canary.finalStart, canary.finalEnd, instance, domain, cell, canary.podallseen1,timestampStart,timestampEnd,ranges, type)));
                futures2.add(pool.submitTask(() -> ArgusQueryT.getStatupAVG(canary.finalStart, canary.finalEnd, instance, domain, cell, canary.podallseen2,timestampStart,timestampEnd,ranges, type)));
                ...
                futures2.add(pool.submitTask(() -> ArgusQueryT.getMetric(ArgusQueryT.avgAPT, canary.finalStart, canary.finalEnd, instance, domain, cell, canary.pod1)));
                futures2.add(pool.submitTask(() -> ArgusQueryT.getMetric(ArgusQueryT.avgAPT, canary.finalStart, canary.finalEnd, instance, domain, cell, canary.pod2)));
                ...
                for (int i = 0; i < metricList.size(); i++) {
                    ...
                    futures2.add(pool.submitTask(() -> ArgusQueryT.getArgusMetric(m, canary.finalStart, canary.finalEnd, instance, domain, cell, canary.pod1)));
                    futures2.add(pool.submitTask(() -> ArgusQueryT.getArgusMetric(m, canary.finalStart, canary.finalEnd, instance, domain, cell, canary.pod2)));
                    ...
                    if (res1 != null && res2 != null) {
                        ...
                        long currentStart = canary.finalStart;
                        while (currentStart <= canary.finalEnd && success) {
                            long currentEnd = currentStart + window;
                            if (currentEnd > canary.finalEnd) {
                                currentEnd = canary.finalEnd; // handle last partial window
                            }

                            System.out.println("Looping for 1:" + instance + ":" + domain + ":" + cell + ":" + Utils.convertEpochToUTCString(currentStart) + ":" + Utils.convertEpochToUTCString(currentEnd));
                            ...
                            futures2.add(pool.submitTask(() -> ArgusQueryT.getArgusMetric(m, currentStarttmp, currentEndtmp, instance, domain, cell, canary.pod1)));
                        ...
                        futures2.clear();
                        if (success) {
                            System.out.println("Looping success for 1:" + instance + ":" + domain + ":" + cell);
                            currentStart = canary.finalStart;;
                            while (currentStart <= canary.finalEnd && success) {
                                long currentEnd = currentStart + window;
                                if (currentEnd > canary.finalEnd) {
                                    currentEnd = canary.finalEnd; // handle last partial window
                                }
                                System.out.println("Looping for 2:" + instance + ":" + domain + ":" + cell + ":" + Utils.convertEpochToUTCString(currentStart) + ":" + Utils.convertEpochToUTCString(currentEnd));
                                ...
                                final long currentStarttmp = currentStart;
                                final long currentEndtmp = currentEnd;
                                futures2.add(pool.submitTask(() -> ArgusQueryT.getArgusMetric(m, currentStarttmp, currentEndtmp, instance, domain, cell, canary.pod2)));
                                currentStart = currentEnd + 1;
                            }
                            for (Future<ArgusQueryT.QueryResponse> future : futures2) {
                                ArgusQueryT.QueryResponse res = future.get();
                        ...
                        if (success) {
                            System.out.println("Looping success for " + instance + ":" + domain + ":" + cell);
                            record.add(metricList.get(i) + "1:number");
                            ...
                            System.out.println("Looping failed for:" + instance + ":" + domain + ":" + cell);
                            record.add(metricList.get(i) + "1:number");
                            record.add(null);
                            header.add(metricList.get(i) + "1:number");
                            record.add(metricList.get(i) + "2:number");
                ...
                futures3.add(pool.submitTask(() -> ArgusQueryT.getHeap(canary.finalStart,canary.finalEnd, instance,domain,cell, canary.pod1)));
                futures3.add(pool.submitTask(() -> ArgusQueryT.getHeap(canary.finalStart,canary.finalEnd, instance,domain,cell, canary.pod2)));

                System.out.println("getInstanceTypeTag start");
                //hack use only 1 hr
                long tmp1 = canary.finalEnd;
                if ((canary.finalEnd - canary.finalStart) > 60 * 60 * 1000) {
                    tmp1 = canary.finalStart + 60 * 60 * 1000;
                }
                List<Future<String>> futures4 = new ArrayList<>();
                final long tmp11 = tmp1;
                futures4.add(pool.submitTask(() -> ArgusQueryT.getInstanceTypeTag(canary.finalStart,tmp11, instance,domain,cell, canary.pod1)));
                futures4.add(pool.submitTask(() -> ArgusQueryT.getInstanceTypeTag(canary.finalStart,tmp11, instance,domain,cell, canary.pod2)));
                ...
                futures4.add(pool.submitTask(() -> ArgusQueryT.getReleaseTag(canary.finalStart,canary.finalEnd, instance,domain,cell, canary.pod1)));
                futures4.add(pool.submitTask(() -> ArgusQueryT.getReleaseTag(canary.finalStart,canary.finalEnd, instance,domain,cell, canary.pod2)));
    ...
    public static CanaryDetails processCanary(String start, String end, String instance, String domain, String cell, String basekpods, String canarykpods) {
        ...
        try {
            ...
            // If both are "*", use original parse method (no filtering)
            // Otherwise, use new parse method with filtering
            if (basekpodsValue.equals("*") && canarykpodsValue.equals("*")) {
                cd = parse(metric, cell);
            } else {
                cd = parseWithFilter(metric, cell, basekpodsValue, canarykpodsValue);
            }
    ...
    /**
     * Original parse method - no filtering, preserves existing functionality
     */
    public static CanaryDetails parse(String metric, String cell) {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            if (type == 0) {
                ...
                // Filter: only apply when basekpods is not "*" (custom handling)
                // If "*", use existing logic (no filtering)
    ...
    /**
     * New parse method with filtering support for basekpods and canarykpods
     * Only applies filtering when basekpods or canarykpods is not "*"
     */
    public static CanaryDetails parseWithFilter(String metric, String cell, String basekpods, String canarykpods) {
        // Default to "*" if not provided
        final String basekpodsValue = (basekpods != null && !basekpods.trim().isEmpty()) ? basekpods : "*";
        final String canarykpodsValue = (canarykpods != null && !canarykpods.trim().isEmpty()) ? canarykpods : "*";

        if(basekpodsValue.equals("*") && canarykpodsValue.equals("*")){
            System.out.println(cell + " both base and canary kpods *");
            return null;
        }
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            //System.out.println("pod: " + tags.get("k8s_pod_name"));
            String pod = tags.get("k8s_pod_name").toString();
            boolean matchesBasePattern = matchesPattern(pod, basekpodsValue);
            boolean matchesCanaryPattern = matchesPattern(pod, canarykpods);
            if(!(matchesBasePattern || matchesCanaryPattern)){
                continue;
            }
    ...
    /**
     * Helper method to check if a pod name matches any pattern in a comma-separated list
     * Supports wildcard matching: "*" matches all, or exact match, or comma-separated list
     * @param podName The pod name to check
     * @param patternList The comma-separated list of patterns to match against (supports "*" wildcard)
     * @return true if pod name matches any pattern in the list
     */
    private static boolean matchesPattern(String podName, String patternList) {
        if (patternList == null || patternList.trim().isEmpty() || patternList.equals("*")) {
            return true;
        }
        ...
        for (String pattern : patterns) {
            ...
            // If pattern is "*", it matches all
            if (pattern.equals("*")) {
                return true;
            }
            // Check exact match
            if (podName.equals(pattern)) {
                return true;
            }
            // Check wildcard pattern: convert "*" to regex
            String regex = pattern.replace("*", ".*");
            if (podName.matches(regex)) {
                return true;
            }
        }
        return false;
